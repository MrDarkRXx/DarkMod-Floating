...
void *Load_Mod(void *) {
    do {
        sleep(1);
    } while (!isLibraryLoaded(OBFUSCATE("libName.so")));

    Options.Example = MemoryPatch(OBFUSCATE("libName.so"), 0x00000,
...
Java_dark_rx_LoaderMod_setDarkStart(JNIEnv *env, jobject thiz, jobject ctx) {
    CheckFloatingPermison(env, ctx);
}

extern "C"
JNIEXPORT jstring JNICALL
Java_dark_rx_Floater_Icon(JNIEnv *env, jobject thiz) {
...
extern "C"
JNIEXPORT void JNICALL
Java_dark_rx_Floater_Changes(JNIEnv *env, jobject thiz, jint feature, jint value, jobject ctx) {
    //Toast Example
//    setDialog(ctx,env,"Title","Message Example");
    //Toast Example
  //  Toast(env,ctx,"By MrDarkkRX",1);
    switch (feature) {
        case 0:
            ex = !ex;
            if (ex){
        ...
        case 1:
            ex2 = !ex2;
            if (ex2){
                Options.Example.Modify();
            } else {
                Options.Example.Restore();
            }
            break;
        case 2:
            if (value == 0){
                 Toast(env,ctx,OBFUSCATE("Value: 0"),1);
            }
            break;
    }
}
...
extern "C"
JNIEXPORT void JNICALL
Java_dark_rx_Floater_StartOptionsDark(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle) {
    //Set Title
    setText(env, title, OBFUSCATE("DarkMod"));
    //Set SubTitle
    setText(env, subtitle, OBFUSCATE("ByMrDarkRX"));
    ...
    pthread_t ptid;
    pthread_create(&ptid, NULL, Load_Mod, NULL);
}